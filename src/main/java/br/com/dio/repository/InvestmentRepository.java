...
import br.com.dio.exception.AccountWithInvestmenException;
...
public class InvestmentRepository {
    ...
    public InvestmentWallet initInvestment(final AccountWallet account, final long id) {
        if (!wallets.isEmpty()) {
            ...
            if (accountInUse.contains(account)) {
                throw new AccountWithInvestmenException("A conta'" + accountInUse + "' já possui um investimento");
            }
        }
    ...
    public InvestmentWallet findWalletByAccountPix(final String pix) {
        return wallets.stream()
                ...
                .findFirst()
                .orElseThrow(
                        () -> new WalletlNotFoundException("A carteira não foi encontrada")
                );
    }
